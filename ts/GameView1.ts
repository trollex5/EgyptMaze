class GameView {
  ...
  private _fogClear:boolean = false;
  ...
  //  console.log("createInterface ShopView");
    ...
    this._eyeOfRa.on('pointerdown', ()=>{
      ...
      if(!this._btnPlay.status){
        ...
        this._innerC.addChild(this._exit.sprite);
  ...
  private calcPos():Array<number> {
    let x:number = (Math.floor(Math.random() * 800) + 180);
    let y:number = (Math.floor(Math.random() * 400) + 100);
